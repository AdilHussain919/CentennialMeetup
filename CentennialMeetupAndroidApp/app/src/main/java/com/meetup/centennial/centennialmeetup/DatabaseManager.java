...
/**
 * Created by Aesha on 11/22/2016.
 */

public class DatabaseManager extends SQLiteOpenHelper {
    //
    private static final String DATABASE_NAME = "CentennialMeetup.db";
    private static final int DATABASE_VERSION = 1;
    //
    private String tables[]; //table names
    private String tableCreatorString[];
    public long res;//SQL statements to create tables
    ...
    long addRecord(ContentValues values, String tableName, String fields[],String record[]) {
       ...
       res = db.insert(tableName, null, values);
        ...
        db.close();
        return res;//close database connection
    }
    ...
    public List getTableColumn(String tableName) {
        List table = new ArrayList(); //to store all rows
        // Select all records
        String selectQuery = "SELECT  user_id FROM " + tableName;
    ...
    public Cursor getUser(String tableName,String centId)
    {
      Cursor cursor = null;
        String centennialId = "";
        SQLiteDatabase db = this.getReadableDatabase();
